...
int main(int argc, char const *argv[])
{
    ...
    float* d_F;
    err = cudaMalloc((void**) &d_F, (2*FILTER_RADIUS+1)*(2*FILTER_RADIUS+1)*sizeof(float));
    cuda_check(err);
    ...
    stbi_write_png("data/img_output/gpu_pinned_benchmark_output_img.png", n_cols, n_rows, 1, ucharData.data(), n_cols);

    // ----------------------------------------------------------------- //
    // ------------------ Saving benchmark results --------------------- //
    // ----------------------------------------------------------------- //
    // Opening File
    const std::string filename = "benchmarking/gpu_pinned.txt";
    ...
    // Writing results
    file << "--------------------------------- \n";
    file << "GPU (Pinned) Benchmarking details \n";
    file << "--------------------------------- \n";
